...
function expiryBadge(days: number) {
  ...
  return { label: `${days}d`, cls: 'bg-slate-800 text-slate-400 border border-slate-700' }
}
...
const CSCS_COLOURS: Record<string, string> = {
  ...
  black: 'bg-slate-700/50 text-slate-300 border-slate-600',
  red:   'bg-red-900/50 text-red-400 border-red-800',
  white: 'bg-slate-800 text-slate-200 border-slate-600',
}
...
const STATUS_COLOURS: Record<string, string> = {
  available: 'text-emerald-400', working: 'text-blue-400', blocked: 'text-red-400',
  verified: 'text-slate-300', unavailable: 'text-slate-500',
  qualifying: 'text-yellow-400', pending_docs: 'text-orange-400', prospect: 'text-slate-600',
}
  ...
  return (
    <div className="p-4 space-y-8">
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Compliance Summary</h2>
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl border border-red-900/60 bg-red-950/20 p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Operatives blocked</p>
            </div>
          </div>
          <div className="rounded-xl border border-orange-900/60 bg-orange-950/20 p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Expiring within 7 days</p>
            </div>
          </div>
          <div className="rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-4 flex items-start gap-3">
            <ShieldCheck className="h-5 w-5 text-emerald-400 shrink-0 mt-0.5" />
            <div>
              <p className="text-2xl font-bold text-emerald-400">{clearOps.length}</p>
              <p className="text-xs text-slate-400 mt-0.5">All docs clear</p>
            </div>
          </div>
        </div>
        ...
        {blockedOps.length > 0 && (
          <div className="rounded-xl border border-slate-800/60 overflow-hidden">
            <div className="px-4 py-2.5 bg-slate-900/80 border-b border-slate-800/60">
              <p className="text-xs font-medium text-red-400 uppercase tracking-wide">Blocked operatives</p>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Reason</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Reemploy</th>
                </tr>
              </thead>
                ...
                {blockedOps.map((op) => (
                  <tr key={op.id} className="hover:bg-slate-900/40">
                    <td className="px-4 py-2.5">
                      <Link href={`/operatives/${op.id}`} className="text-emerald-400 hover:underline font-medium">
                        {op.first_name} {op.last_name}
                      </Link>
                    </td>
                    <td className="px-4 py-2.5 text-slate-400 text-xs">{op.blocked_reason ?? '—'}</td>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">
          Expiring Documents
          <span className="ml-2 font-normal text-slate-500 normal-case">(next 90 days)</span>
        </h2>
        {docs.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60 bg-slate-900/60">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Document</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Expires</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Remaining</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/40">
                {docs.map((doc) => {
                  ...
                  return (
                    <tr key={doc.id} className="hover:bg-slate-900/40">
                      <td className="px-4 py-2.5">
                        {op
                          ? <Link href={`/operatives/${op.id}`} className="text-emerald-400 hover:underline font-medium">{op.first_name} {op.last_name}</Link>
                          : <span className="text-slate-500">Unknown</span>}
                      </td>
                      <td className="px-4 py-2.5 text-slate-300">{DOC_LABELS[doc.document_type] ?? doc.document_type}</td>
      ...
      {/* ── 3. RIGHT TO WORK ──────────────────────────────────────────────── */}
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Right to Work</h2>

        {/* RTW stats */}
        <div className="grid grid-cols-4 gap-3">
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className="text-2xl font-bold text-emerald-400 tabular-nums">{rtwVerified.length}<span className="text-sm font-normal text-slate-500 ml-1">/ {ops.length}</span></p>
            <p className="text-xs text-slate-500 mt-0.5">Verified ({rtwPct}%)</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${rtwUnverified.length > 0 ? 'text-red-400' : 'text-slate-500'}`}>{rtwUnverified.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Not verified</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className="text-2xl font-bold text-sky-400 tabular-nums">{rtwTimeLimited.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Time-limited</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${rtwExpiring30.length > 0 ? 'text-orange-400' : 'text-slate-500'}`}>{rtwExpiring30.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Expiring ≤30 days</p>
          </div>
        </div>

        {/* RTW table — needs attention only */}
        {rtwNeedsAction.length === 0 ? (
          <div className="rounded-xl border border-emerald-900/40 bg-emerald-950/10 py-6 text-center text-sm text-emerald-500 flex items-center justify-center gap-2">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60 bg-slate-900/40">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">RTW Type</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Verified</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Expiry</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">GOV.UK Check</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/40">
                {rtwNeedsAction.map((op) => {
                  ...
                  return (
                    <tr key={op.id} className="hover:bg-slate-900/40">
                      ...
                      <td className="px-4 py-2.5">
                        {isIndefinite
                          ? <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-900/40 text-emerald-400 border border-emerald-800">Indefinite</span>
                          : op.rtw_expiry
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className="text-2xl font-bold text-emerald-400 tabular-nums">{cscsHave.length}<span className="text-sm font-normal text-slate-500 ml-1">/ {ops.length}</span></p>
            <p className="text-xs text-slate-500 mt-0.5">Hold CSCS ({cscsPct}%)</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${cscsMissing.length > 0 ? 'text-red-400' : 'text-slate-500'}`}>{cscsMissing.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">No card recorded</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${cscsExpiring.length > 0 ? 'text-orange-400' : 'text-slate-500'}`}>{cscsExpiring.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Expiring ≤90 days</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${cscsExpired.length > 0 ? 'text-red-400' : 'text-slate-500'}`}>{cscsExpired.length}</p>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(cardTypeCounts).filter(([k]) => k !== 'none').map(([type, count]) => (
            <span key={type} className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${CSCS_COLOURS[type] ?? 'bg-slate-800 text-slate-400 border-slate-700'}`}>
              {type.charAt(0).toUpperCase() + type.slice(1)} card <span className="font-bold">{count}</span>
            </span>
          ))}
          {(cardTypeCounts['none'] ?? 0) > 0 && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-slate-800/60 text-slate-500 border-slate-700">
          ...
          <div className="rounded-xl border border-slate-800/60 overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60 bg-slate-900/40">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Card Type</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Expiry</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Remaining</th>
                </tr>
              </thead>
                ...
                {cscsNeedsAction.map((op) => {
                  ...
                  return (
                    <tr key={op.id} className="hover:bg-slate-900/40">
                      ...
                      <td className="px-4 py-2.5">
                        {op.cscs_card_type
                          ? <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${CSCS_COLOURS[op.cscs_card_type] ?? 'bg-slate-800 text-slate-400 border-slate-700'}`}>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide flex items-center gap-2">
          ...
          <span className="font-normal text-slate-500 normal-case">({currentWeekStart} week)</span>
        </h2>

        {/* WTD summary stats */}
        <div className="grid grid-cols-4 gap-3">
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className="text-2xl font-bold text-slate-300 tabular-nums">{currentWeekSheets.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Timesheets this week</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${wtdOver48.length > 0 ? 'text-red-400' : 'text-slate-500'}`}>{wtdOver48.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Over 48h this week</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${wtdWarn.length > 0 ? 'text-amber-400' : 'text-slate-500'}`}>{wtdWarn.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">40–48h (approaching)</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${wtdShifts.length > 0 ? 'text-orange-400' : 'text-slate-500'}`}>{wtdShifts.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Shift violations (28d)</p>
          </div>
        </div>
        ...
        {currentWeekSheets.length === 0 ? (
          ...
          <div className="rounded-xl border border-slate-800/60 overflow-hidden">
            <div className="px-4 py-2.5 bg-slate-900/60 border-b border-slate-800/60">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Weekly hours — current week</p>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60 bg-slate-900/40">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Hours</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs w-48">Progress</th>
              ...
              <tbody className="divide-y divide-slate-800/40">
                {currentWeekSheets.sort((a, b) => (b.total_hours ?? 0) - (a.total_hours ?? 0)).map((sheet) => {
                  ...
                  const barCls = hours > 48 ? 'bg-red-500' : hours >= 40 ? 'bg-amber-500' : 'bg-emerald-500'
                  const statusCls = hours > 48 ? 'text-red-400' : hours >= 40 ? 'text-amber-400' : 'text-emerald-400'
                  const statusLabel = hours > 48 ? 'Over limit' : hours >= 40 ? 'Approaching' : 'OK'
                  return (
                    <tr key={sheet.id} className="hover:bg-slate-900/40">
                      <td className="px-4 py-2.5">
                        {op
                          ? <Link href={`/operatives/${op.id}`} className="text-emerald-400 hover:underline font-medium">{op.first_name} {op.last_name}</Link>
                          : <span className="text-slate-500">—</span>}
                      </td>
                      <td className="px-4 py-2.5 tabular-nums font-medium text-slate-200">
                        {hours.toFixed(1)}h
                        {sheet.overtime_hours ? <span className="text-slate-500 text-xs ml-1">({sheet.overtime_hours.toFixed(1)} OT)</span> : null}
                      </td>
                      <td className="px-4 py-2.5">
                        <div className="h-1.5 w-full rounded-full bg-slate-700 overflow-hidden">
                          <div className={`h-full rounded-full transition-all ${barCls}`} style={{ width: `${pct}%` }} />
                        </div>
                        <p className="text-[10px] text-slate-600 mt-0.5 tabular-nums">{pct.toFixed(0)}% of 48h</p>
                      </td>
                      <td className="px-4 py-2.5">
          ...
          <div className="rounded-xl border border-slate-800/60 overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60 bg-slate-900/40">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Shift</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Violations</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/40">
                {wtdShifts.map((shift) => {
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">
          Workforce Status
          <span className="ml-2 font-normal text-slate-500 normal-case">({ops.length} operatives)</span>
        </h2>
        <div className="grid grid-cols-4 gap-3">
          {Object.entries(statusCounts).sort((a, b) => b[1] - a[1]).map(([status, count]) => (
            <div key={status} className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-4">
              <p className={`text-2xl font-bold tabular-nums ${STATUS_COLOURS[status] ?? 'text-slate-300'}`}>{count}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800/60 bg-slate-900/60">
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Status</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Trade</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Engagement</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Compliance</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/40">
              {ops.map((op) => {
                ...
                return (
                  <tr key={op.id} className="hover:bg-slate-900/40">
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`text-xs font-medium ${STATUS_COLOURS[op.status ?? ''] ?? 'text-slate-400'}`}>
                        {STATUS_LABELS[op.status ?? ''] ?? op.status}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-slate-400 text-xs">{trade?.name ?? '—'}</td>
                    <td className="px-4 py-2.5 text-slate-400 text-xs capitalize">
                    ...
                    <td className="px-4 py-2.5">
                      ...
                      {!op.compliance_alert && <span className="text-xs text-emerald-400">Clear</span>}
                    </td>
                  </tr>
                )
              })}
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide flex items-center gap-2">
          ...
          <span className="font-normal text-slate-500 normal-case">({ncrList.length} total)</span>
        </h2>
        <div className="grid grid-cols-4 gap-3">
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className="text-2xl font-bold text-slate-300 tabular-nums">{ncrList.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Total NCRs</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${ncrUnresolved.length > 0 ? 'text-red-400' : 'text-slate-500'}`}>{ncrUnresolved.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Unresolved</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className={`text-2xl font-bold tabular-nums ${(ncrBySeverity['critical'] ?? 0) > 0 ? 'text-red-400' : 'text-slate-500'}`}>{ncrBySeverity['critical'] ?? 0}</p>
            <p className="text-xs text-slate-500 mt-0.5">Critical severity</p>
          </div>
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className="text-2xl font-bold text-slate-400 tabular-nums">{ncrList.length - ncrUnresolved.length}</p>
        ...
        {Object.keys(ncrByType).length > 0 && (
          <div className="flex flex-wrap gap-2">
            {Object.entries(ncrByType).sort((a, b) => b[1] - a[1]).map(([type, count]) => (
              <span key={type} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-slate-800/60 text-slate-400 border-slate-700">
                {type.replace(/_/g, ' ')} <span className="font-bold text-slate-300">{count}</span>
              </span>
            ))}
          </div>
        )}
        ...
        {ncrUnresolved.length > 0 && (
          <div className="rounded-xl border border-slate-800/60 overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60 bg-slate-900/40">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Ref</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Site</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Type</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Severity</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/40">
                {ncrUnresolved.map((n) => {
                  ...
                  return (
                    <tr key={n.id} className="hover:bg-slate-900/40">
                      <td className="px-4 py-2.5 text-slate-500 text-xs tabular-nums">{n.reference_number ?? '—'}</td>
                      <td className="px-4 py-2.5">
                        {op
                          ? <Link href={`/operatives/${op.id}`} className="text-emerald-400 hover:underline font-medium">{op.first_name} {op.last_name}</Link>
                          : <span className="text-slate-500">—</span>}
                      </td>
                      <td className="px-4 py-2.5 text-slate-400 text-xs">{site?.name ?? '—'}</td>
                      <td className="px-4 py-2.5 text-slate-400 text-xs capitalize">{n.incident_type.replace(/_/g, ' ')}</td>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide flex items-center gap-2">
          ...
          <span className="font-normal text-slate-500 normal-case">({reviewList.length} reviews)</span>
        </h2>
        <div className="grid grid-cols-4 gap-3">
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
            <p className="text-2xl font-bold text-slate-300 tabular-nums">{reviewList.length}</p>
          ...
          <div className="rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-3">
            <p className="text-2xl font-bold text-emerald-400 tabular-nums">{rapTrafficCounts['green'] ?? 0}</p>
            <p className="text-xs text-slate-500 mt-0.5">Green</p>
          </div>
          <div className="rounded-xl border border-amber-900/60 bg-amber-950/20 p-3">
            ...
            <p className="text-xs text-slate-500 mt-0.5">Amber</p>
          </div>
          <div className="rounded-xl border border-red-900/60 bg-red-950/20 p-3">
            ...
            <p className="text-xs text-slate-500 mt-0.5">Red</p>
          </div>
        </div>
        ...
        {rapRows.length > 0 && (
          <div className="rounded-xl border border-slate-800/60 overflow-hidden">
            <div className="px-4 py-2.5 bg-slate-900/60 border-b border-slate-800/60">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Average scores per operative (lowest first)</p>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60 bg-slate-900/40">
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Operative</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Reviews</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs w-48">Avg Score</th>
                  <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Last Review</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/40">
                {rapRows.map((r) => {
                  ...
                  const barCls = avg == null ? 'bg-slate-700' : avg < 2.5 ? 'bg-red-500' : avg < 3.5 ? 'bg-amber-500' : 'bg-emerald-500'
                  const scoreCls = avg == null ? 'text-slate-500' : avg < 2.5 ? 'text-red-400' : avg < 3.5 ? 'text-amber-400' : 'text-emerald-400'
                  return (
                    <tr key={r.opId} className="hover:bg-slate-900/40">
                      <td className="px-4 py-2.5">
                        <Link href={`/operatives/${r.opId}`} className="text-emerald-400 hover:underline font-medium">{r.name}</Link>
                      </td>
                      <td className="px-4 py-2.5 text-slate-400 tabular-nums text-xs">{r.count}</td>
                      <td className="px-4 py-2.5">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">
            Allocation History
            <span className="ml-2 font-normal text-slate-500 normal-case">(last 200)</span>
          </h2>
          {canExport && (
            <a href="/api/reports/allocations-csv" className="flex items-center gap-1.5 text-xs text-emerald-400 hover:text-emerald-300 transition-colors">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800/60 bg-slate-900/60">
                ...
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Site</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Trade</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Start</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">End</th>
                <th className="text-right px-4 py-2.5 text-slate-500 font-medium text-xs">Rate</th>
                <th className="text-left px-4 py-2.5 text-slate-500 font-medium text-xs">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/40">
              {allocs.map((al) => {
                ...
                return (
                  <tr key={al.id} className="hover:bg-slate-900/40">
                    ...
                    <td className="px-4 py-2.5 text-slate-300 text-xs">{site?.name ?? '—'}</td>
                    <td className="px-4 py-2.5 text-slate-400 text-xs">{lr?.trade_category?.name ?? '—'}</td>
                    ...
                    <td className="px-4 py-2.5 text-slate-400 tabular-nums text-xs">
                      {al.end_date ? new Date(al.end_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: '2-digit' }) : <span className="text-slate-600">Ongoing</span>}
                    </td>
                    <td className="px-4 py-2.5 text-right tabular-nums text-slate-300 text-xs">
                      {al.agreed_day_rate != null ? `£${Number(al.agreed_day_rate).toFixed(0)}` : '—'}
                    </td>
                    <td className="px-4 py-2.5 text-xs capitalize text-slate-400">
