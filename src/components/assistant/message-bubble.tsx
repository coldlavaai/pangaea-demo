...
function SimpleMarkdown({ text }: { text: string }) {
  ...
  while (i < lines.length) {
    ...
    // Headings
    if (line.startsWith('### ')) {
      elements.push(<p key={i} className="font-semibold text-muted-foreground text-xs mt-2 mb-0.5">{renderInline(line.slice(4))}</p>)
    } else if (line.startsWith('## ')) {
      elements.push(<p key={i} className="font-semibold text-foreground text-sm mt-2 mb-0.5">{renderInline(line.slice(3))}</p>)
    } else if (line.startsWith('# ')) {
      elements.push(<p key={i} className="font-semibold text-foreground mt-2 mb-0.5">{renderInline(line.slice(2))}</p>)
    }
    // Bullet list
    else if (line.startsWith('- ') || line.startsWith('• ')) {
      elements.push(<p key={i} className="pl-3 text-muted-foreground">• {renderInline(line.slice(2))}</p>)
    }
    // Numbered list
    else if (/^\d+\.\s/.test(line)) {
      elements.push(<p key={i} className="pl-3 text-muted-foreground">{renderInline(line)}</p>)
    }
    // Horizontal rule
    else if (line.trim() === '---') {
    ...
    // Normal paragraph
      ...
      elements.push(<p key={i} className="text-muted-foreground">{renderInline(line)}</p>)
    }
    i++
  }
  ...
  return (
    <div className={cn('flex', isUser ? 'justify-end' : 'justify-start')}>
      <div className={cn('max-w-[85%]', isUser ? 'max-w-[70%]' : 'w-full')}>
        {isUser ? (
          <div className="bg-forest-600/20 border border-forest-600/30 rounded-2xl rounded-tr-sm px-3 py-2">
          ...
          <div className="space-y-1">
            ...
            {!message.content && !message.richBlocks?.length && message.isStreaming && (
              <div className="bg-card/60 rounded-2xl rounded-tl-sm px-3 py-2">
            ...
            {message.content && (
              <div className="bg-card/60 rounded-2xl rounded-tl-sm px-3 py-2">
                ...
                {message.isStreaming && (
                  <span className="inline-block w-1.5 h-3.5 bg-forest-400 animate-pulse ml-0.5 align-text-bottom" />
                )}
              </div>
            )}
