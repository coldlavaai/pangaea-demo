...
export function AssistantWidget() {
  ...
  // Listen for toggle-alf custom event from sidebar
  useEffect(() => {
    const handler = () => setOpen(prev => !prev)
    window.addEventListener('toggle-alf', handler)
    return () => window.removeEventListener('toggle-alf', handler)
  }, [])
    ...
    <div className="fixed bottom-5 right-5 z-50">
      <div className="w-[380px] h-[500px] flex flex-col bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 border-b border-slate-800">
          ...
          <span className="text-xs font-semibold text-slate-200 flex-1">ALF</span>
          {messages.length > 0 && (
            <button
